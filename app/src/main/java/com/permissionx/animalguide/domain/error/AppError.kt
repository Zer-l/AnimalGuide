...
sealed class AppError : Exception() {
    data class NetworkError(override val message: String = "网络连接失败，请检查网络后重试") :
        AppError()

    data class TimeoutError(override val message: String = "请求超时，请检查网络后重试") : AppError()
    data class ParseError(override val message: String = "未找到该动物的相关信息，请检查名称是否正确") :
        AppError()

    data class NotAnimalError(override val message: String = "未识别到动物，请换一张更清晰的图片重试") :
        AppError()
    ...
    data class UserNotFoundError(override val message: String = "用户不存在") : AppError()

    data class AuthExpiredError(override val message: String = "登录已过期") : AppError()
}
